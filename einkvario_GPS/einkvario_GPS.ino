...
GxEPD2_BW<GxEPD2_213_B72, GxEPD2_213_B72::HEIGHT> display(GxEPD2_213_B72(/*CS=5*/ SS, /*DC=*/ 17, /*RST=*/ 16, /*BUSY=*/ 4)); // GxEPD2_213_B73
...
void  updateDisplay(void * pvParameters)
{
  while (1) {
    //更新显示 280X122
    display.fillRect(0, 0, 104, 25, GxEPD_BLACK);
    display.fillRect(disOff + 30, 25, 74, 187, GxEPD_WHITE);

    xSemaphoreTake(mutex_v, portMAX_DELAY);
    display.setTextColor(GxEPD_BLACK);
    display.setFont(&FreeSerifBold18pt7b);
    if (fabs(vspeed) < 10)
      displayData(vspeed, disOff + 40, 55, 1);
    displayData(gps.speed.kmph(), disOff + 40, 90, 0); //speed
    displayData(gps.altitude.meters(), disOff + 30, 130);
    display.setFont(&FreeSerifBold12pt7b);
    displayData(atmAlti, disOff + 30, 160,1);//altitude1.get() 
    displayData(temperature, disOff + 40, 190);
    displayData(gps.satellites.value(), disOff + 80, 190);
    display.setCursor(disOff + 30,210);
    //display.print(String(hour())+':'+String(minute())+':'+String(second()));
    display.print(gps.time.value()+8e6);
    ...
    if(deviceConnected)
      ...
      u8g2Fonts.print("蓝牙");
    u8g2Fonts.setCursor(disOff + 35,20);
    u8g2Fonts.print("音量"+String(isound));
    display.setCursor(disOff + 75,20);
    ...
    display.hibernate();
    //display.powerOff();
    vTaskDelayUntil(&xPreWkTmDisplay,pdMS_TO_TICKS(1000));
  }
}
...
void setup()
{
  ...
  ss.begin(9600,SWSERIAL_8N1, rxPin, 55, false,2048);//
  ...
  u8g2Fonts.setFont(u8g2_font_wqy16_t_gb2312a);            // select u8g2 font from here: https://github.com/olikraus/u8g2/wiki/
   ...
   display.fillRect(disOff, 0, 120, 25, GxEPD_BLACK);
  u8g2Fonts.setForegroundColor(GxEPD_WHITE);
  u8g2Fonts.setCursor(disOff, 20);
  u8g2Fonts.print("蓝牙");
  u8g2Fonts.setForegroundColor(GxEPD_BLACK);
  u8g2Fonts.setBackgroundColor(GxEPD_WHITE);
  u8g2Fonts.setCursor(disOff, 50);
  u8g2Fonts.print("升速");
  u8g2Fonts.setCursor(disOff, 85);
  u8g2Fonts.print("速度");
  u8g2Fonts.setCursor(disOff, 140);
  u8g2Fonts.print("高度");
  u8g2Fonts.setCursor(disOff, 190);
  u8g2Fonts.print("温度");
